    ...
    .exec((err, findCa) => {
      if (err) {
        ...
        if (findCa) {
          ...
          categories.create(newCategorie, (err, createC) => {
            if (err) {
              const error = {
                ok: false,
                status: 400,
...
services.getSearchCheck = data => new Promise((resolve, reject) => {
  categories.findById(data)
    ...
    .exec((err, finCa) => {
      if (err) {
        reject(new Error({ ok: false, status: 500, message: 'Internal serve error', err }))
      } else {
  ...
  subCategories.findOne({
    ...
    if (err) {
      ...
      if (findSub) {
        ...
        subCategories.create(subCatego, (err, success) => {
          if (err) {
            reject(new Error({
              ok: false,
              status: 400,
